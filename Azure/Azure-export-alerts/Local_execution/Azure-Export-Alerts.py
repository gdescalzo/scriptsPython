...
def load_credentials(json_filepath):
    ...
    credentials = data['sre_credentials'][0]
...
filepath = r'C:\SomePath\SRE_Credentials.json'
tenant_id, client_id, client_secret = load_credentials(filepath)
...
alert_pattern = 'Alert pattern name'

# Please input the Azure Subscription Pattern
suscription_pattern = 'Some pattern'

# Please input the Excel file name
today = datetime.today().strftime('%Y-%m-%d')  # Get current date in YYYY-MM-DD format

excel_file_name = 'Some name' # Please input the Excel file name without the extension 
excel_file_name_export = f'{excel_file_name}-{today}.xlsx' # Name of the Excel file,  with the current date 

# Get the subscription list
subscription_names = list_subscriptions_by_name(credentials, suscription_pattern)
...
dataframe.to_excel(excel_file_name_export, index=False)
full_path = os.path.join(os.getcwd(), excel_file_name_export)
